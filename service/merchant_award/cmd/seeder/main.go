...
import (
	...
	"github.com/MamangRust/microservice-ecommerce-grpc-merchant_award/seeder"
...
func main() {
	...
	q, closeFn, err := open(logger, "DB_MERCHANT_AWARD")
	if err != nil {
		logger.Fatal("Failed to connect to database", zap.Error(err))
	}
	defer closeFn()

	s := seeder.NewMerchantAwardSeeder(q, ctx, logger)
	if err := s.Seed(); err != nil {
		logger.Fatal("Failed to seed merchant awards", zap.Error(err))
	}

	logger.Info("merchant awards seeded successfully")
}
