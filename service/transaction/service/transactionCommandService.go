...
func (s *transactionCommandService) Create(ctx context.Context, req *requests.CreateTransactionRequest) (*db.CreateTransactionRow, error) {
	...
	if s.outbox != nil {
		...
		// Stats pipeline (F3): publish a stats.ecommerce.transaction.event so
		// stats-writer materializes this transaction into ClickHouse for the
		// stats-reader endpoints. The envelope carries event_id for idempotent
		// consumption and the business event_time for ReplacingMergeTree.
		...
		if statsErr != nil {
			status = "error"
			return errorhandler.HandleError[*db.CreateTransactionRow](s.logger, statsErr, method, span)
		}
...
// mustJSON marshals a plain struct payload. Stats event payloads contain only
// scalar JSON fields, so marshaling cannot fail; this helper keeps the outbox
// block readable.
