...
const getUserByEmailAndVerify = `-- name: GetUserByEmailAndVerify :one
SELECT
    user_id,
    firstname,
    lastname,
...
func (q *Queries) GetUserByEmailAndVerify(ctx context.Context, email string) (*GetUserByEmailAndVerifyRow, error) {
	row := q.db.QueryRow(ctx, getUserByEmailAndVerify, email)
	var i GetUserByEmailAndVerifyRow
	err := row.Scan(
		&i.UserID,
		&i.Firstname,
		&i.Lastname,
...
const getUserByIdAndVerify = `-- name: GetUserByIdAndVerify :one
...
type GetUserByIdAndVerifyRow struct {
	UserID int32  `json:"user_id"`
	Email  string `json:"email"`
}
...
func (q *Queries) GetUserByIdAndVerify(ctx context.Context, userID int32) (*GetUserByIdAndVerifyRow, error) {
	row := q.db.QueryRow(ctx, getUserByIdAndVerify, userID)
	var i GetUserByIdAndVerifyRow
...
func (q *Queries) GetUserByVerificationCode(ctx context.Context, verificationCode string) (*GetUserByVerificationCodeRow, error) {
	row := q.db.QueryRow(ctx, getUserByVerificationCode, verificationCode)
	var i GetUserByVerificationCodeRow
	err := row.Scan(
		&i.UserID,
		&i.Firstname,
		&i.Lastname,
