...
func (r *merchantBusinessSeeder) Seed() error {
	// Idempotency: skip when business information already exists.
	existing, err := r.db.GetMerchantsBusinessInformation(r.ctx, db.GetMerchantsBusinessInformationParams{
		Column1: "",
		Limit:   1,
		Offset:  0,
	})
	if err == nil && len(existing) > 0 {
		r.logger.Debug("merchant business information already seeded, skipping")
		return nil
	}
