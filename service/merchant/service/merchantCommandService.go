...
func (s *merchantCommandService) Create(ctx context.Context, request *requests.CreateMerchantRequest) (*db.CreateMerchantRow, error) {
	...
	if s.pool != nil {
		...
		if err == nil && s.outbox != nil {
			err = s.outbox.EnqueueInTx(ctx, NewOutboxQuerier(db.New(tx)), "email-service-topic-merchant-create", strconv.Itoa(int(res.MerchantID)), payloadBytes)
		}
		if err != nil {
			status = "error"
			return errorhandler.HandleError[*db.CreateMerchantRow](
	...
	if s.pool != nil {
		...
		if err == nil && subject != "" && s.outbox != nil {
			...
			if marshalErr != nil {
				...
				err = s.outbox.EnqueueInTx(ctx, NewOutboxQuerier(db.New(tx)), "email-service-topic-merchant-update-status", strconv.Itoa(int(res.MerchantID)), payloadBytes)
			}
		}
		if err != nil {
			status = "error"
