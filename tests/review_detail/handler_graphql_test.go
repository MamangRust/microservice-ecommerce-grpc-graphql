...
func (s *ReviewDetailGraphQLTestSuite) TearDownSuite() {
	s.BaseTestSuite.TearDownSuite()
}

func gqlReviewDetail(h http.Handler, query string, variables map[string]interface{}) map[string]interface{} {
...
func (s *ReviewDetailGraphQLTestSuite) Test1_SmokeQuery() {
	// Basic smoke test - just verify GraphQL handler works
	result := gqlReviewDetail(s.graphqlH, "{ __typename }", nil)
	s.NotNil(result)
}
