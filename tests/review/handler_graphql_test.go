...
func (s *ReviewGraphQLTestSuite) TearDownSuite() {
	s.BaseTestSuite.TearDownSuite()
}

func gqlReview(h http.Handler, query string, variables map[string]interface{}) map[string]interface{} {
...
func (s *ReviewGraphQLTestSuite) Test1_SmokeQuery() {
	// Basic smoke test - just verify GraphQL handler works
	result := gqlReview(s.graphqlH, "{ __typename }", nil)
	s.NotNil(result)
}
