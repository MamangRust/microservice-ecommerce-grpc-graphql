...
func gqlOrderItem(h http.Handler, query string, variables map[string]interface{}) map[string]interface{} {
...
func (s *OrderItemGraphQLTestSuite) Test1_SmokeQuery() {
	// Basic smoke test - just verify GraphQL handler works
	result := gqlOrderItem(s.graphqlH, "{ __typename }", nil)
	s.NotNil(result)
}
