...
func gqlProduct(h http.Handler, query string, variables map[string]interface{}) map[string]interface{} {
...
func (s *ProductGraphQLTestSuite) Test1_SmokeQuery() {
	// Basic smoke test - just verify GraphQL handler works
	result := gqlProduct(s.graphqlH, "{ __typename }", nil)
	s.NotNil(result)
}
