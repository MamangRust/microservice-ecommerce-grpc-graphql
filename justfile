...
# package (service/<name>/database/schema) built from its own migrations and
# queries (service/<name>/database/{migration,query}/sqlc.yaml).
generate-sql:
    for y in service/*/database/sqlc.yaml; do (cd "$$(dirname $$y)" && sqlc generate) || exit 1; done
