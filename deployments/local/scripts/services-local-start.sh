...
# Each service runs its own migrations at startup from ./migrations (same
# layout as the Docker image), so cwd must be the service directory.
for svc in "${SERVICES[@]}"; do
  setsid nohup bash -c "cd '$PROJECT_ROOT/service/$svc' && go run ./cmd/main.go > '$LOG_DIR/$svc.log' 2>&1" > /dev/null 2>&1 &
  echo "started $svc"
done
