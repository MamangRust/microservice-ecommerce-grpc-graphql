...
| **API Framework** | gqlgen (GraphQL) | Type-safe GraphQL API Gateway (Playground at `/`, query endpoint `/query`) |
...
| Command | Description |
|---------|-------------|
| `just build-up` | Build service images lalu start local Compose |
| `just up` | Start local Compose menggunakan image yang tersedia |
| `just down` | Stop dan remove container Compose |
| `docker compose -f deployments/local/docker-compose.yml ps` | Lihat status container |
| `just migrate` | Jalankan migration database ke versi terbaru |
| `just migrate-down` | Rollback satu migration |
| `just seeder` | Jalankan database seeder |
| `just generate-proto` | Regenerate Go code dari `.proto` |
| `sqlc generate` | Regenerate Go code dari SQL query |
| `just generate-swagger` | Regenerate Swagger API documentation |
| `just build-image` | Build image semua service sesuai daftar recipe |
| `just test-unit` | Jalankan test package `pkg` |
| `just test-integration` | Jalankan integration test dari module `tests` |
| `just test-all` | Jalankan unit dan integration test |

Target Kubernetes harus dijalankan menggunakan manifest dan tool cluster yang
tersedia; command `make kube-*` tidak tersedia sebagai source of truth di
repository ini.
...
<p align="center">
  Built with Go, GraphQL, gRPC, and a passion for clean microservice architecture.
</p>